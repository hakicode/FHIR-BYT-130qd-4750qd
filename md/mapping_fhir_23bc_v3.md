...
| `Encounter.reasonCode[1].text` | string | ⚪ | `LY_DO_VNT` 🆕 | B1(23), Checkin(15) | **Chuỗi(1024)** | **QĐ4750 bổ sung** — lý do vào cấp cứu; hỗ trợ xác định ca cấp cứu bên cạnh `MA_DOITUONG_KCB=2` |
...
| `Encounter.hospitalization.dischargeDisposition` | CodeableConcept | ⚪ | `KET_QUA_DTRI` | B1(41) | Số(1) | Kết quả điều trị — phân loại hồ sơ khỏi/đỡ/tử vong/xin về |
...
| `Patient.address[0].postalCode` | string | ⚪ | `MAXA_CU_TRU` 🔄 | B1(15) | **Chuỗi(3)** | **QĐ 4750 giảm size 5→3** — cần lookup mã xã cũ→mới khi migration |
...
| `Encounter.serviceProvider` | Reference | ⚪ | `MA_CSKCB` | B1(58) | Chuỗi(5) | Breakdown thống kê theo đơn vị CSKCB |
...
|---|---|---|---|---|
| `Condition.id` / `EpisodeOfCare.id` | `MA_LK` | B1(1) | Chuỗi(100) | PRIMARY KEY liên kết đợt điều trị — khóa GROUP BY khi đếm ca mắc |
...
|---|---|---|---|---|---|
| `Encounter.id` | id | `MA_LK` | B1(1) | Chuỗi(100) | PRIMARY KEY — đơn vị đếm và tổng hợp ngày điều trị |
...
| `Encounter.period.end` | dateTime | ⚪ | `NGAY_RA` | B1(37) | Chuỗi(12) | Xác định kỳ đóng hồ sơ — dùng khi group chi phí theo ngày ra viện |
| `Claim.total.value` | decimal | ⚪ | `T_TONGCHI_BV` | B1(47) | Số(15) | Tổng chi phí BV theo đợt — dùng cho báo cáo tổng hợp theo ngày thanh toán |
...
| Trường FHIR | Kiểu FHIR | Trường XML | Bảng XML | Kiểu XML | Ghi chú |
|---|---|---|---|---|---|
| `Encounter.id` | id | `MA_LK` | B1(1) | Chuỗi(100) | PRIMARY KEY — đơn vị đếm nhập/ra viện |
| `Encounter.period.start` | dateTime | `NGAY_VAO` | B1(35) | Chuỗi(12) | Ngày vào viện — kỳ báo cáo |
...
| `Claim.total.value` | decimal | ⚪ | `T_TONGCHI_BV` | B1(47) | Số(15) | Tổng chi phí BV theo đợt ra viện trong ngày |
| `ClaimResponse.payment.amount.value` | decimal | ⚪ | `T_BHTT` | B1(51) | Số(15) | Số tiền BHXH thanh toán theo đợt ra viện |
